...
int precision(int f = 40, int n = 1000000) {
    ...
    for (int i = 0; i < prec_n; ++i) {
        ...
        for (std::vector<int>::iterator limit = limits.begin(); limit != limits.end(); ++limit) {
            ...
            t_start = std::chrono::high_resolution_clock::now();
            t.get_nns_by_item(j, (*limit), (size_t) -1, &toplist,
            ...
            std::vector<int>::iterator it_set = std::set_intersection(closest.begin(), closest.end(), toplist.begin(),
        ...
        for (std::vector<int>::iterator limit = limits.begin(); limit != limits.end(); ++limit) {
            std::cout << "limit: " << (*limit) << "\tprecision: " << std::fixed << std::setprecision(2)
                      << (100.0 * prec_sum[(*limit)] / (i + 1)) << "% \tavg. time: " << std::fixed
                      << std::setprecision(6) << (time_sum[(*limit)] / (i + 1)) * 1e-04 << "s" << std::endl;
        }
